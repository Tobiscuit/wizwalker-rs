...
impl TeleportHelper {
    ...
    pub async fn write_position(&self, xyz: &XYZ) -> Result<()> {
        self.inner.write_value_to_offset(0, xyz)
    }

    pub async fn should_update(&self) -> Result<bool> {
        self.inner.read_value_from_offset(12)
    }

    pub async fn write_should_update(&self, should_update: bool) -> Result<()> {
        self.inner.write_value_to_offset(12, &should_update)
    }

    pub async fn target_object_address(&self) -> Result<u64> {
        self.inner.read_value_from_offset(13)
    }

    pub async fn write_target_object_address(&self, target_object_address: u64) -> Result<()> {
