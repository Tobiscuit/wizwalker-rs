...
impl<R: MemoryReader + 'static> GameClient<R> {
    ...
    fn pattern_scan_offset_cached(&self, pattern: &[u8], offset_idx: usize, default: u64) -> Result<u64> {
    ...
    pub fn elastic_camera_controller(&self) -> Result<Option<u64>> {
    ...
    pub fn free_camera_controller(&self) -> Result<Option<u64>> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn selected_camera_controller(&self) -> Result<Option<u64>> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn write_selected_camera_controller(&self, selected_camera_controller: u64) -> Result<()> {
        let offset = self.pattern_scan_offset_cached(
            b"\x48\x89\x87\x08\x20\x02\x00\x48\x8D\x8F\x10\x20\x02\x00\x48\x8D\x54\x24\x40\xE8\x00\x00\x00\x00\x90\x48\x8B\x4C\x24",
            3,
            0x22290
    ...
    pub fn is_freecam(&self) -> Result<bool> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn write_is_freecam(&self, is_freecam: bool) -> Result<()> {
        let offset = self.pattern_scan_offset_cached(
            b"\x0F\xB6\x88\x20\x20\x02\x00\x88\x8B\x6A\x02\x00\x00\x84\xC9\x0F\x85\x00\x00\x00\x00\x48\x8D\x55\xE0\x48\x8B\xCB\xE8",
            3,
            0x222A8
    ...
    pub fn root_client_object(&self) -> Result<Option<ClientObject<R>>> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn frames_per_second(&self) -> Result<f32> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn shutdown_signal(&self) -> Result<i32> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn write_shutdown_signal(&self, shutdown_signal: i32) -> Result<()> {
        let offset = self.pattern_scan_offset_cached(
            b"\x38\x9F\xB8\x11\x02\x00\x74\xBE\xE8\x00\x00\x00\x00\x83\xF8\x64\x0F\x8F\x00\x00\x00\x00\xB9\x0F\x00\x00\x00",
            2,
            0x211B8
    ...
    pub fn character_registry(&self) -> Result<Option<u64>> {
        let offset = 0x224A8;
        let addr = self.read_value_from_offset::<u64>(offset)?;
        if addr == 0 {
            return Ok(None);
    ...
    pub fn account_permissions(&self) -> Result<AccountPermissions> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn write_account_permissions(&self, account_permissions: AccountPermissions) -> Result<()> {
        let offset = self.pattern_scan_offset_cached(
            b"\x41\x89\x86\x3C\x1D\x02\x00\x4D\x8B\x06\x8B\xD0\x49\x8B\xCE\x41\xFF\x90\x10\x04\x00\x00\x49\x8B\x06\x49\x8B\xCE\xFF\x90\x58\x01\x00\x00",
            3,
            0x21D3C
    ...
    pub fn write_has_membership(&self, has_membership: bool) -> Result<()> {
        let offset = self.pattern_scan_offset_cached(
            b"\x83\xBB\x40\x1D\x02\x00\x00\x75\x04\xB2\x01\xEB\x02\x33\xD2\x48\x8B\x00\x00\x00\x00\x00\xE8",
            2,
            0x21D40
    ...
    pub fn gamebryo_presenter(&self) -> Result<Option<u64>> {
        let offset = self.pattern_scan_offset_cached(
    ...
    pub fn fishing_manager(&self) -> Result<Option<u64>> {
        let offset = 0x23140;
        let addr = self.read_value_from_offset::<u64>(offset)?;
        if addr == 0 {
            return Ok(None);
...
impl<R: MemoryReader + 'static> CurrentGameClient<R> {
    ...
    pub fn get_client(&self) -> Result<GameClient<R>> {
