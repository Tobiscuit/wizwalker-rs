...
use crate::automation::camera_utils::{point_to_xyz_orient, OrbitParams, GlideParams, rotation_velocity};
...
pub async fn parse_location(s: &[CommandToken], cam: Option<&DynamicCameraController>, cl: Option<&Client>) -> (Vec<XYZ>, Vec<Orient>) {
    let s: Vec<String> = s.iter().map(|x| x.as_str().to_lowercase().replace(", ", "")).collect();
    ...
    else if let Some(c) = cl { (c.body().position().unwrap_or_default(), c.body().orientation().unwrap_or_default()) }
    else { (XYZ::default(), Orient::default()) };
    for arg in s {
        if arg.contains("xyz") {
            let split: Vec<&str> = arg.replace("xyz(", "").trim_end_matches(')').split(',').collect();
            ...
            let split: Vec<&str> = arg.replace("orient(", "").trim_end_matches(')').split(',').collect();
...
pub fn client_from_titles(cls: &[Client], t: &str) -> Option<Client> { cls.iter().find(|c| c.title().to_lowercase() == t.to_lowercase()).cloned() }
...
async fn use_potion(c: &Client) { if let Some(s) = c.stats() { if s.potion_charge().unwrap_or(0.0) >= 1.0 { click_window_by_path(c, paths::POTION_USAGE_PATH).await; } } }
...
pub async fn parse_command(clients: Vec<Client>, command_str: &str) -> Result<(), String> {
    let all_clients = clients.clone(); let mut cmd_str = command_str.replace(", ", ",");
    ...
    match cmd_f.as_str() {
        ...
        _ => {
            let mut clients = clients; let mut split = split;
            ...
            if !c_str.contains("mass") {
                ...
                if !c_str.is_empty() && c_str.chars().nth(1).map_or(false, |c| c.is_numeric()) {
                    if excl { clients = all_clients.into_iter().filter(|c| !prov.iter().any(|pc| pc.title() == c.title())).collect(); }
                    else { clients = prov; }
                }
            }
            ...
            match split[1].as_str().to_lowercase().as_str() {
                "teleport" | "tp" | "setpos" => {
                    ...
                    match split[2].as_str().to_lowercase().as_str() {
                        "closestmob" | "mob" => { join_all(clients.iter().map(|c| async move { let s = SprintyClient::new(c); let m = s.get_mobs(); if let Some(i) = s.find_closest_of(&m) { if let Some(l) = m[i].location { c.teleport(l).await.ok(); } } })).await; }
                        "quest" | "questpos" | "questposition" => { if !clients.is_empty() { if let Some(qp) = clients[0].quest_position().and_then(|q| q.position()) { join_all(clients.iter().map(|c| c.teleport(qp))).await; } } }
                        _ => {
                            let mut loc = None; for p in &all_clients { if p.title() == split[2].as_str() { loc = Some(p.body().position().unwrap_or_default()); break; } }
                            if let Some(l) = loc { join_all(clients.iter().map(|c| c.teleport(l))).await; }
                            else {
                                let mut xyzs = Vec::new(); for c in &clients { let (cx, _) = parse_location(&split, None, Some(c)).await; xyzs.push(cx.get(0).cloned().unwrap_or_default()); }
                                join_all(clients.iter().zip(xyzs).map(|(c, x)| c.teleport(x))).await;
                            }
                        }
                    }
                }
                "walkto" | "goto" => {
                    let mut xyzs = Vec::new(); for c in &clients { let (cx, _) = parse_location(&split, None, Some(c)).await; xyzs.push(cx.get(0).cloned().unwrap_or_default()); }
                    join_all(clients.iter().zip(xyzs).map(|(c, x)| c.goto(x.x, x.y))).await;
                }
                "sendkey" | "press" | "presskey" => {
                    if split.len() >= 3 {
                        ...
                        if let Some(kv) = k { join_all(clients.iter().map(|c| c.send_key(kv, t))).await; }
                    }
                }
                "waitfordialog" | "waitfordialogue" => {
                    join_all(clients.iter().map(|c| async move { wait_for_coro(|| async { c.is_in_dialog() }, false, 0.25).await; })).await;
                    if split[1].as_str().to_lowercase() == "completion" { join_all(clients.iter().map(|c| async move { wait_for_coro(|| async { c.is_in_dialog() }, true, 0.25).await; })).await; }
                }
                "waitforbattle" | "waitforcombat" => {
                    join_all(clients.iter().map(|c| async move { wait_for_coro(|| async { c.in_battle() }, false, 0.25).await; })).await;
                    if split.last().unwrap().as_str().to_lowercase() == "completion" { join_all(clients.iter().map(|c| async move { wait_for_coro(|| async { c.in_battle() }, true, 0.25).await; })).await; }
                }
                "waitforzonechange" | "wait_for_zone_change" => {
                    if split.len() >= 4 && split[split.len()-2].as_str().to_lowercase() == "from" { let f = split.last().unwrap().as_str(); join_all(clients.iter().map(|c| async move { wait_for_zone_change(c, Some(f), 60.0); })).await; }
                    else if split.len() >= 4 && split[split.len()-2].as_str().to_lowercase() == "to" { let t = split.last().unwrap().as_str(); join_all(clients.iter().map(|c| async move { while c.zone_name().unwrap_or_default() != t { tokio::time::sleep(tokio::time::Duration::from_millis(250)).await; } })).await; }
                    else {
                        join_all(clients.iter().map(|c| async move { let s = c.zone_name().unwrap_or_default(); while c.zone_name().unwrap_or_default() == s { tokio::time::sleep(tokio::time::Duration::from_millis(100)).await; } })).await;
                        if split.last().unwrap().as_str().to_lowercase() == "completion" { join_all(clients.iter().map(|c| async move { wait_for_coro(|| async { c.is_loading() }, true, 0.25).await; })).await; }
                    }
                }
                "waitforfree" => {
                    join_all(clients.iter().map(|c| async move { while !is_free(c) { tokio::time::sleep(tokio::time::Duration::from_millis(250)).await; } })).await;
                    if split.last().unwrap().as_str().to_lowercase() == "completion" { join_all(clients.iter().map(|c| async move { while is_free(c) { tokio::time::sleep(tokio::time::Duration::from_millis(250)).await; } })).await; }
                }
                "usepotion" => {
                    if split.len() > 3 {
                        let h = split[2].as_str().parse::<i32>().unwrap_or(20); let m = split[3].as_str().parse::<i32>().unwrap_or(10);
                        join_all(clients.iter().map(|c| async move { if let Some(st) = c.stats() { let hr = st.current_hitpoints().unwrap_or(1) as f32 / st.max_hitpoints().unwrap_or(1) as f32; let mr = st.current_mana().unwrap_or(1) as f32 / st.max_mana().unwrap_or(1) as f32; if hr * 100.0 <= h as f32 || mr * 100.0 <= m as f32 { use_potion(c).await; } } })).await;
                    } else { join_all(clients.iter().map(|c| use_potion(c))).await; }
                }
                "buypotions" | "refillpotions" | "buypots" | "refillpots" => {
                    if split.len() > 2 && split[2].as_str() == "ifneeded" { join_all(clients.iter().map(|c| async move { if let Some(st) = c.stats() { if st.potion_charge().unwrap_or(0.0) < 1.0 { refill_potions(c, false, true).await; } } })).await; }
                ...
                "waitforwindow" | "waitforpath" => {
                    if split.len() >= 3 && split[2].is_list() {
                        let p_v = split[2].as_list(); let p: Vec<&str> = p_v.iter().map(|s| s.as_str()).collect();
                        join_all(clients.iter().map(|c| async move { while !is_visible_by_path(c, &p) { tokio::time::sleep(tokio::time::Duration::from_millis(250)).await; } })).await;
                        if split.last().unwrap().as_str().to_lowercase() == "completion" { join_all(clients.iter().map(|c| async move { while is_visible_by_path(c, &p) { tokio::time::sleep(tokio::time::Duration::from_millis(250)).await; } })).await; }
                    }
                }
                "friendtp" | "friendteleport" => {
                    tokio::time::sleep(tokio::time::Duration::from_millis(250)).await;
                    if split.len() >= 3 && split[2].as_str() == "icon" { join_all(clients.iter().map(|c| teleport_to_friend_from_list(c, None, Some(2)))).await; }
                    else { let n = split[2..].iter().map(|t| t.as_str()).collect::<Vec<_>>().join(" "); join_all(clients.iter().map(|c| teleport_to_friend_from_list(c, Some(n.clone()), None))).await; }
                }
                "entitytp" | "entityteleport" => { if split.len() >= 3 { let n = split[2].as_str(); join_all(clients.iter().map(|c| async move { let s = SprintyClient::new(c); let e = s.get_entities_with_vague_name(n); if let Some(i) = s.find_closest_of(&e) { if let Some(l) = e[i].location { c.teleport(l).await.ok(); } } })).await; } }
                "tozone" | "to_zone" => { if split.len() >= 3 { let z = split[2].as_str(); join_all(clients.iter().map(|c| async move { while c.zone_name().unwrap_or_default() != z { tokio::time::sleep(tokio::time::Duration::from_millis(1000)).await; } })).await; } }
...
pub async fn execute_flythrough(c: &Client, data: &str, sep: &str) -> Result<(), String> {
    ...
    if let Some(gc) = c.game_client() { if let Ok(f) = gc.is_freecam() { if !f { c.camera_freecam().await.ok(); } } }
    if let Some(gc) = c.game_client() { if let Some(cam) = gc.free_camera_controller() { for a in cmds { parse_camera_command(&cam, &a).await; } } }
    Ok(())
}
...
pub async fn parse_camera_command(cam: &DynamicCameraController, s: &str) {
    ...
    match cmd.as_str() {
        "glideto" => {
            ...
            while start.elapsed().as_secs_f32() < t { let dt = start.elapsed().as_secs_f32(); let c_xyz = glide.position_at(&orig_p, dt); cam.write_position(c_xyz).ok(); if let Some(f) = f_xyz { cam.update_orientation(point_to_xyz_orient(&c_xyz, &f)).ok(); } else { cam.update_orientation(t_o).ok(); } tokio::time::sleep(tokio::time::Duration::from_millis(10)).await; }
            cam.write_position(t_xyz).ok();
        }
        "rotatingglideto" => {
            ...
            while start.elapsed().as_secs_f32() < t { cam.write_position(glide.position_at(&orig_p, start.elapsed().as_secs_f32())).ok(); c_o.pitch += rot_v.pitch * 0.01; c_o.roll += rot_v.roll * 0.01; c_o.yaw += rot_v.yaw * 0.01; cam.update_orientation(c_o).ok(); tokio::time::sleep(tokio::time::Duration::from_millis(10)).await; }
            cam.write_position(t_xyz).ok();
        }
        "orbit" => {
            ...
            while start.elapsed().as_secs_f32() < t { let dt = start.elapsed().as_secs_f32(); let c_xyz = orb.position_at(dt); cam.write_position(c_xyz).ok(); cam.update_orientation(orb.orientation_at(&c_xyz, orig_o.roll)).ok(); tokio::time::sleep(tokio::time::Duration::from_millis(10)).await; }
        }
        "lookat" => { if let Some(t_x) = handle_index(&xyzs, 0, None) { cam.update_orientation(point_to_xyz_orient(&orig_p, &t_x)).ok(); } }
        "setpos" => { if let Some(t_x) = handle_index(&xyzs, 0, None) { cam.write_position(t_x).ok(); } }
        "setorient" => { if let Some(t_o) = handle_index(&orients, 0, None) { cam.update_orientation(t_o).ok(); } }
        _ => {}
    }
}
