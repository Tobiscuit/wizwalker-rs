# Math Glossary Generator
# Uses Gemini CLI in headless mode to generate a glossary for each unit
# Structure: math-glossary/{course}/{unit}/glossary.md
...
$baseDir = "$env:USERPROFILE\Desktop\math-glossary"
$totalUnits = 0

Write-Host ""
Write-Host "========================================" -ForegroundColor Cyan
Write-Host "  Math Glossary Generator" -ForegroundColor Cyan
Write-Host "  Powered by Gemini CLI" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""

# Count total
foreach ($course in $courses.GetEnumerator()) { $totalUnits += $course.Value.Count }
Write-Host "Courses: $($courses.Count) | Total Units: $totalUnits" -ForegroundColor Yellow
...
foreach ($course in $courses.GetEnumerator() | Sort-Object Key) {
    ...
    foreach ($unit in $course.Value) {
        $unitDir = Join-Path $courseDir $unit
        New-Item -ItemType Directory -Path $unitDir -Force | Out-Null
    }
    
    $courseName = ($course.Key -replace '^\d+-', '') -replace '-', ' '
    Write-Host "  Created: $($course.Key)/ ($($course.Value.Count) units)" -ForegroundColor Gray
}

Write-Host ""
Write-Host "Folder structure created! Now generating glossaries..." -ForegroundColor Green
Write-Host ""
...
foreach ($course in $courses.GetEnumerator() | Sort-Object Key) {
    $courseName = ($course.Key -replace '^\d+-', '') -replace '-', ' '
    $courseTitle = (Get-Culture).TextInfo.ToTitleCase($courseName)
    
    foreach ($unit in $course.Value) {
        $current++
        $unitName = ($unit -replace '^\d+-', '') -replace '-', ' '
        $unitTitle = (Get-Culture).TextInfo.ToTitleCase($unitName)
        
        $unitDir = Join-Path (Join-Path $baseDir $course.Key) $unit
        $glossaryFile = Join-Path $unitDir "glossary.md"
        
        # Skip if already generated
        if (Test-Path $glossaryFile) {
            Write-Host "  [$current/$totalUnits] SKIP: $courseTitle > $unitTitle (exists)" -ForegroundColor DarkGray
            continue
        }
        
        Write-Host "  [$current/$totalUnits] " -ForegroundColor Cyan -NoNewline
        Write-Host "$courseTitle > $unitTitle" -ForegroundColor White -NoNewline
        Write-Host " ..." -ForegroundColor Gray -NoNewline
        
        $prompt = @"
Create a comprehensive glossary for the following math unit. This is for a developer learning mathematics.

Course: $courseTitle
Unit: $unitTitle

Format the glossary as a Markdown document with:
1. A # header with the unit name
2. A brief (2-3 sentence) overview of what this unit covers and why it matters
3. A ## Glossary section with every key term, symbol, and concept in this unit
4. Each term should have:
   - The term in **bold**
   - A clear, concise definition (1-2 sentences)
   - The mathematical notation/symbol if applicable (in LaTeX: $notation$)
   - A simple example or analogy (especially useful for developers — relate to programming concepts where possible)
5. A ## Key Formulas section listing the essential formulas with LaTeX notation
6. A ## Connections section (2-3 sentences) explaining how this unit connects to the next unit and to programming/CS

Keep it reference-style — concise but complete. A developer should be able to ctrl+F this document to look up any term they encounter while studying.

Print ONLY the Markdown content, nothing else.
"@
        
        try {
            gemini $prompt 2>$null | Out-File -FilePath $glossaryFile -Encoding utf8
            $lines = (Get-Content $glossaryFile | Measure-Object -Line).Lines
